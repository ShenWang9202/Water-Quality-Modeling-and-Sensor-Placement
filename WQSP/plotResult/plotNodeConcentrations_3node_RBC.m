function plotNodeConcentrations_3node_RBC(NodeID4Legend,X_node_control_result)
figure1 = figure
h1 = subplot(1,2,1)
fontsize = 38;
plot(X_node_control_result,'LineWidth',1);
xticks([0 360 720 1080 1440])
% yticks([0 0.8 2 3])
xlim([-30,1440])
ymin = min(min(X_node_control_result));
ymax = max(max(X_node_control_result));
ylim([ymin,ymax]);
...
plot(ranges,X_node_control_result(ranges,:),'LineWidth',2);
xticks([200 260])
...
print(figure1,'EnlargedNodeConcentrations_RBC','-depsc2','-r300');
end